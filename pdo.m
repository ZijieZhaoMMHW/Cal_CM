function [pdo_p,pdo_idx,lon_pdo,lat_pdo]=pdo(sst,t,lon,lat,land_mask);
...
t_size=size(sst_anom,3);
area_global=repmat((cosd(lat))',360,1);

sst_anom_used=sst_anom(lon<=240 & lon>=120, lat>=20 & lat<=60,:);
area_used=area_global(lon<=240 & lon>=120, lat>=20 & lat<=60,:);
lon_pdo=lon(lon<=240 & lon>=120);
lat_pdo=lat(lat>=20 & lat<=60);
sst_rm_global=NaN(size(sst_anom_used));

for i=1:size(sst_anom_used,3);
    global_ssta=sst_anom(:,:,i).*area_global;
    global_ssta=nanmean(global_ssta(:));
    sst_rm_global(:,:,i)=sst_anom_used(:,:,i)-global_ssta;
end

[eof_full,pc_full,expvar_full] = eof(sst_rm_global.*repmat(area_used,1,1,t_size),t_size,'mask',~isnan(land_mask(lon<=240 & lon>=120, lat>=20 & lat<=60)));

if find(expvar_full==nanmax(expvar_full))~=length(expvar_full)
    pdo_idx=pc_full(1,:)./nanstd(pc_full(1,:));
    pdo_p=eof_full(:,:,1).*nanstd(pc_full(1,:));
else
    pdo_idx=-pc_full(end,:)./nanstd(pc_full(end,:));
    pdo_p=-eof_full(:,:,end).*nanstd(pc_full(end,:));
end
